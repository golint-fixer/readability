...
var automatedReadabilityIndexTests = []struct {
	text     string
	expected string
}{
	{
...
func TestAutomatedReadabilityIndex(t *testing.T) {
	for _, test := range automatedReadabilityIndexTests {
		got := fmt.Sprintf("%.3f", AutomatedReadabilityIndex(test.text))
		if got != test.expected {
			t.Errorf("Expected %s score, got %s for: [%s]", test.expected, got, test.text[:10])
		}
	}
}
